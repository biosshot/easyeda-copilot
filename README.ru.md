...
Соберите `skill/` из актуального MCP командой `npm ci && npm run build:skill`, затем установите runtime-зависимости по [install-guide.md](mcp/docs/install-guide.md). Сгенерированная папка не коммитится. Альтернатива — архив скилла под нужную платформу из Releases, уже содержащий зависимости. Прежняя установка MCP не меняется.
