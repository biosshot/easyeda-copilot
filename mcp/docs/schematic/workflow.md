...
3. Search only unknown parts with `component_search` and use explicit, resolved components. It searches the `lcsc` library by default. Call `library_list` when a public-library component is needed, then pass its alias or explicit public library UUID as `library_uuid`. Each result with numeric or blank pin names has `preview_recommended: true` and a local PNG `preview_image_path`; open that path to inspect the symbol. Use `preview_component({ part_uuid })` to render any candidate explicitly. Read pin numbers on the image and verify uncertain pin functions against the exact part datasheet before connecting them.
