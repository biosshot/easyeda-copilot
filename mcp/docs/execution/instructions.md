...
`file_path` is an absolute path on the **MCP host**. The MCP server reads the UTF-8 file and sends its text through Copilot's existing connection. It is not a path inside EasyEDA. Code is limited to 1 MiB of UTF-8; empty code and non-regular files are rejected. Files can contain normal newlines, comments and `await`.
...
Open and inspect the exact document first with Copilot's document tools. Every execution, including reads and syntax errors, creates a checkpoint before compiling/running the script. If checkpoint creation fails, the code does not run. New checkpoints are matched to the document UUID, type and parent project/library; restoring one into another document is refused. Older snapshots without a verifiable target are not restored automatically.
...
The extension encodes binary data for transport; the MCP process decodes and writes it to its local temporary directory, `easyeda-copilot-mcp/responses`. Base64 and byte arrays are not printed into the response. Open a returned image with the available image-viewing tool. Artifacts are temporary: copy an output into the project if the user needs to keep it.

The maximum inline **serialized MCP tool result** is 16,384 UTF-8 bytes, including escaping, content blocks, `structuredContent`, metadata and errors. The limit is fixed and applies across Copilot tools; there is no input flag to bypass it.
...
- For another oversized tool result (including SDK validation errors), the JSON artifact contains its complete MCP result envelope. The inline notice identifies this format. Text JSON may therefore be inside `content[].text` in that file.
