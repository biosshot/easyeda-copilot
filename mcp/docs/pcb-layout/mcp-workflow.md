...
5. If routing is required, call `run_auto_route_on_current_pcbdoc` on the same opened PCB document.

Do not assemble for a standalone schematic, an unrelated PCB, or an ambiguous project structure.
</MCP_PCB_ASSEMBLY_WORKFLOW>

<MCP_PCB_AUTOROUTE_WORKFLOW>
Use `run_auto_route_on_current_pcbdoc` after placement assembly when the board should be routed inside EasyEDA.

The tool exports EasyEDA autoroute JSON, runs the custom router on the MCP side, imports the routed JSON back into EasyEDA, then rebuilds GND copper pours and adds GND SUTURE vias.
...
Only override these values when the user gives manufacturing or RF/power-layout constraints.
</MCP_PCB_AUTOROUTE_WORKFLOW>
