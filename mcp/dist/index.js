...
// ../packages/shared/types/circuit.ts
import { z as z4 } from "zod";

// ../packages/shared/types/lcsc.ts
import { z as z2 } from "zod";
var LCSC_uuid = () => z2.string().regex(/^[0-9a-f]{32}$/);

// ../packages/shared/types/reused.ts
import { z as z3 } from "zod";

// ../packages/shared/types/circuit.ts
var PinSchema = () => z4.object({
  pin_number: z4.union([z4.number(), z4.string()]).describe("Pin number."),
  name: z4.string().describe('Pin name (e.g., "VCC").'),
  signal_name: z4.string().describe("The name of the signal the pin is connected to. (Name only). The signal name assigned to the pin must be identical to the signal name of the target output.")
});
var BaseComponentSchema = () => z4.object({
  designator: z4.string().describe('Component identifier (e.g., "U1", "R5", "J1", "X1").'),
  value: z4.string().describe('Minimum description: for simple components \u2014 only the nominal value; for microcircuits \u2014 only the name. Only ASCII symbols (e.g., "LM358", "10nF", "100k").'),
  pins: z4.array(PinSchema()).describe("Pin details."),
  block_name: z4.string().describe("Reference to the block."),
  search_query: z4.string().describe('A component search question. For example: "1k 1W smd resistor", "LM358", "2-pin power connector"'),
...
var CircuitReusedBlockSchema = () => z4.object({
  block_uuid: z4.string(),
  parameters_to_recalc: z4.array(z4.object({
    name: z4.string(),
    new_value: z4.number()
  })).describe("Parameters that will be recalculated to suit your needs"),
  ports: z4.array(z4.object({
    port_number: z4.string().or(z4.number()).describe("Port number"),
    signal_name: z4.string().describe("Port signal_name")
  }))
});
var CircuitModStruct = () => z4.object({
  add_components: z4.array(BaseComponentSchema().omit({ part_uuid: true }).extend({
    part_uuid: LCSC_uuid().describe("part_uuid of the lcsc component")
  })).describe("Components to add"),
  add_reused_blocks: z4.array(CircuitReusedBlockSchema()).describe("reuded blocks to add"),
  rm_components: z4.array(z4.string().describe("component designator")).nullable().describe("Components to remove from the circuit"),
  external_rm_connect: z4.array(z4.object({
    designator: z4.string().describe("Target component designator"),
    pin_number: z4.union([z4.number(), z4.string()]).describe("Target component pin number")
  ...
  external_connect: z4.array(z4.object({
    designator: z4.string().describe("Target component designator"),
    pin_number: z4.union([z4.number(), z4.string()]).describe("Target component pin number"),
    signal_name: z4.string().describe("Signal name")
...
// src/index.ts
var COPILOT_SERVER_URL = (process.env.EASYEDA_COPILOT_SERVER_URL || "https://circuit.tech.ru.net").replace(/\/$/, "");
var MCP_WS_PORT = Number(process.env.EASYEDA_COPILOT_MCP_WS_PORT || 8787);
var MCP_WS_HOST = process.env.EASYEDA_COPILOT_MCP_WS_HOST || "127.0.0.1";
...
async function requestEasyEda(event, body = {}, timeoutMs = 12e4) {
...
async function postJson(path, payload) {
  ...
  if (!response.ok) {
    const message = typeof data === "object" && data && "error" in data ? String(data.error) : `HTTP ${response.status}`;
    throw new Error(message);
  }
  return data;
}
...
server.registerTool(
  "read_prompt",
  {
    ...
    inputSchema: z5.object({
      name: z5.enum(["skill_agent", "circuit_maker", "combined"]).describe("Prompt to read.")
    })
  },
  async ({ name }) => {
    if (name === "skill_agent") {
...
server.registerTool(
  "component_search",
  {
    ...
    inputSchema: z5.object({
      part_uuid: z5.string().nullable().optional(),
      MPN: z5.string().nullable().optional()
    })
  },
  async ({ part_uuid, MPN }) => {
    if (!part_uuid && !MPN) {
...
server.registerTool(
  "search_reused_block",
  {
    ...
    inputSchema: z5.object({
      query: z5.string().describe('Query example: "3.3V power regulator"'),
      page: z5.number().min(1).default(1).describe("Current results page."),
      limit: z5.number().min(1).max(25).default(10).describe("Number of results per page.")
    })
  },
...
server.registerTool(
  ...
  async (circuit) => {
    const missingPartUuid = circuit.add_components.filter((component) => !component.part_uuid || /^0+$/.test(component.part_uuid)).map((component) => component.designator);
    ...
    await requestEasyEda("assemble-circuit", result, 3e5);
...
server.registerTool(
  ...
  async () => {
    const result = await requestEasyEda("get-schematic");
    return textResult({ ...result, components: result.components.map((c) => ({ ...c, pos: void 0 })) });
  }
);
server.registerTool(
...
server.registerTool(
  "open_document",
  {
    ...
    inputSchema: z5.object({
      document_uuid: z5.string().min(1).describe("Document UUID from get_current_project_info.")
    })
  },
  async ({ document_uuid }) => {
...
server.registerTool(
  "create_schematic",
  {
    ...
    inputSchema: z5.object({
      parent_board_name: z5.string().min(1).optional().describe("Optional parent board name.")
    })
  },
...
server.registerTool(
  "create_schematic_page",
  {
    ...
    inputSchema: z5.object({
      schematic_uuid: z5.string().min(1).describe("Parent schematic UUID.")
    })
  },
...
server.registerTool(
  "modify_schematic_name",
  {
    ...
    inputSchema: z5.object({
      schematic_uuid: z5.string().min(1).describe("Schematic UUID."),
      schematic_name: z5.string().min(1).describe("New schematic name.")
    })
  },
...
server.registerTool(
  "modify_schematic_page_name",
  {
    ...
    inputSchema: z5.object({
      schematic_page_uuid: z5.string().min(1).describe("Schematic page UUID."),
      schematic_page_name: z5.string().min(1).describe("New schematic page name.")
    })
  },
