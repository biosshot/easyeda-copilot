...
run([npmCli, 'install', '--ignore-scripts', '--no-audit', '--no-fund', '--prefer-offline', '--fetch-timeout=30000', '--fetch-retries=0', '--registry=https://registry.npmjs.org', backendPack.archive, mcpPack.archive], consumer);
...
copyFileSync(join(repo, 'mcp/scripts/check-local-backend.mjs'), join(consumer, 'mcp-smoke.mjs'));
process.stdout.write(run([join(consumer, 'mcp-smoke.mjs'), join(consumer, 'node_modules/easyeda-copilot-mcp/dist/index.js'), join(consumer, 'fixtures.mjs')], consumer, cleanEnv));
