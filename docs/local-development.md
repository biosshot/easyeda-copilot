...
Финальный `post-placement` использует тот же пул процессов для оценки кандидатов одной итерации. `PCB_LAYOUT_SUBTREE_WORKERS=0` или `1` включает последовательный refine. Варианты одной пары компонентов делят кеш baseline; итоговый ход выбирается в исходном порядке. Профиль `PCB_BOARD_PACKER_PROFILE=1` также показывает число кандидатов и отсечений, вызовы роутера, время подготовки данных в TypeScript и нативных вызовов. Суммарное время работы процессов отличается от общего времени итерации.
