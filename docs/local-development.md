...
При вызове `make_pcb_layout` MCP задаёт `PCB_LAYOUT_SUBTREE_WORKERS` по числу доступных логических ядер (`availableParallelism()`). Локальный backend использует отдельные процессы для независимых блоков; `0` отключает этот пул. Общий поиск платы и независимые геометрические оценки используют потоки Rust: `PCB_BOARD_PACKER_THREADS` по умолчанию также равен числу доступных ядер. Фактическое число работников ограничено числом независимых задач. Оба значения можно переопределить в окружении. `PCB_BOARD_PACKER_PROFILE=1` включает раздельные таймеры поиска, локального улучшения и исправления пересечений. После обновления нужно пересобрать native backend, MCP/скилл и перезапустить демон; опубликованная версия backend может ещё не содержать эти изменения.
