...
При вызове `make_pcb_layout` MCP включает параллельный расчёт независимых частей платы в опубликованном backend: по умолчанию `PCB_LAYOUT_SUBTREE_WORKERS=4` (backend ограничивает число потоков числом доступных ядер минус одно). Значение можно переопределить в окружении процесса MCP; `0` отключает этот пул. Для уже запущенного MCP или CLI daemon нужна пересборка MCP и перезапуск процесса.
