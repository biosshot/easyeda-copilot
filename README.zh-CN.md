...
2. 检查原理图分组和完整连接关系，以识别初步功能模块以及局部无源器件归属。
3. 检查当前板框、封装、布局、铜层和 DRC 规则。
4. 在布局 DSL 中描述机械、功能和电气意图。
5. 审查机械预览和最终布局预览。
6. 在已打开的 EasyEDA PCB 文档中组装获准的布局。
7. 在布线 DSL 中定义叠层、布线规则、网络类、平面、特殊网络、扇出和过孔缝合。
8. 将布线程序作为一个由检查点保护的事务应用。
9. 检查关键网络、剩余连接、铜对象以及原生 DRC 结果。
10. 保留结果、进行定向修复，或恢复上一个检查点。
